...
`micro/dense_coordinate_stages.d` isolates arithmetic used while reconstructing
DenseNodes IDs and coordinates. It generates three encoded `sint64` delta
streams for ID, latitude and longitude. Their deterministic delta patterns,
ZigZag encoding and varint encoding match the corresponding payload streams
generated by `micro/dense_nodes.d`.

The diagnostic does not build DenseNodes protobuf field framing. The production
DenseNodes benchmark additionally wraps those payload streams as length-delimited
fields 1, 8 and 9 before constructing the surrounding DenseNodes and
PrimitiveGroup messages.

The five measured stages are:

- `sint64-decode`: decode all three streams with `sint64` varint plus ZigZag
  decoding, without cumulative reconstruction;
- `delta-unchecked`: decode the same streams and reconstruct ID, latitude and
  longitude with ordinary integer addition;
- `delta-checked`: perform the same reconstruction with `checkedAdd`;
- `coords-unchecked`: use ordinary delta accumulation followed by direct affine
  nanodegree reconstruction;
- `coords-checked`: use the same ordinary delta accumulation followed by
  nanodegree reconstruction with `checkedMulAdd`.

The two delta stages therefore differ only in ordinary versus checked cumulative
addition. The two coordinate stages use identical ordinary delta accumulation
and differ only in direct affine arithmetic versus `checkedMulAdd`.

Before timing, the benchmark verifies that the checked and unchecked delta
stages produce the same checksum and that the checked and unchecked coordinate
stages produce the same checksum.

Five cyclic stage orders are rotated between samples. Every stage occupies every
measurement position once during each complete five-sample cycle.
`--iterations` must be exactly `1`.

The measured stages exclude protobuf field scanning, tags, DenseInfo, node-view
construction, workload generation and reporting.

Run the D diagnostic:

```bash
D_OSM_BENCH_COMPILERS=ldc2 \
./benchmark/run-dense-coordinate-stages.sh \
    --nodes=200000 --iterations=1 --samples=30 --warmup=10
```

`reference/dense_coordinate_stages_cpp.cpp` implements the corresponding C++20
diagnostic with the same three encoded input streams, five stage semantics,
paired checksum validation and five cyclic stage orders.

Run the C++ diagnostic:

```bash
./benchmark/run-dense-coordinate-stages-cpp.sh \
    --nodes=200000 --iterations=1 --samples=30 --warmup=10
```

This is a diagnostic arithmetic benchmark, not an end-to-end parser benchmark.
A production optimization suggested by it must still be measured separately
through the production `decodeDenseNodes` benchmark.
